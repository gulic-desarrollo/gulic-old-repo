...
"""BOT creado para enlazar una ventana  en Drupal, con
un servidor jabber y una base de datos MySQL."""
...
class MsnTransport:
    ...
    class MsnChatListener(msnp.ChatCallbacks):
        ...
        def message_received(
            ...
            print 'msn::chat::message_received(passportid=%s, name=%s, text=%s, charset=%s)'\
        ...
        def friend_joined(self, passport_id, displayname):
            ...
            print 'msn::chat::friend_joined(%s, %s)' % (passport_id,
                    displayname)
        ...
        def friend_online(
            ...
            print 'msn::chat::friend_online(%s, %s, %s)' % (state,
    ...
    def broadcastMessage__erroneo(self, text, fullname=''):
        ...
        if len(nuevosChats):
            ...
            for i in self.msn.active_chats:
                try:
                    print 'Intentando enviar a ', \
                        self.msn.active_chats[i].passport_id
                    nuevosChats.index(self.msn.active_chats[i].passport_id)
                    print 'Enviando mensaje a: %s'\
                         % self.msn.active_chats[i].passport_id
                    self.msn.active_chats[i].send_message(text)
                    print 'dos'
...
class JabberBase:
    ...
    def __init__(
        ...
        realserver = 'jabber.gulic.org'  # esta en pdnsd
    ...
    def _messageCB(self, sess, mess):
        ...
        ts2secs = lambda ts: time.mktime(time.strptime(ts,
                '%Y%m%dT%H:%M:%S'))
        ...
        if (time.time() - 3600 * time.daylight) - ts > 30:
            print 'ignorando mensaje antiguo'
        ...
        if tipo == 'error':
            return 'ignorando error desconocido'
        ...
        if type == 'subscribe':
            ...
            if self.room != '' and self.room in who:
                if resource in self.muc.contactos:
                    ...
                    print 'No consegui borrar muc contacto ', resource, \
                        ' de ', self.muc.contactos
            else:
                if jid in self.roster.contactos:
                    self.roster.contactos.remove(jid)
                else:
                    print 'No consegui borrar roster contacto ', jid, \
                        ' de ', self.muc.contactos
            ...
            print u'%s is available (%s / %s)' % (who, prs.getShow(),
                    prs.getStatus())
            if self.room != '' and self.room in who:
                if not self.muc:
                    ...
                    print 'anadiendo (online) ', resource, ' a ', \
                        self.muc.contactos
                else:
                    print 'no anadiendo (online) ', resource, ' a ', \
                        self.muc.contactos
            else:
                if not jid in self.roster.contactos:
                    self.roster.contactos.append(jid)
                    print 'anadiendo (online) ', jid, ' a ', \
                        self.roster.contactos
                else:
                    print 'no anadiendo (online) ', jid, ' a ', \
                        self.roster.contactos
        elif type == 'unavailable':

            print u'%s is unavailable (%s / %s)' % (who, prs.getShow(),
                    prs.getStatus())
            if self.room != '' and self.room in who:
                ...
                if not self.muc:
                    ...
                    self.muc.contactos.remove(resource)
                    print 'anadiendo (offline) ', resource, ' de ', \
                        self.muc.contactos
                else:
                    print 'no anadiendo (offline) ', resource, ' de ', \
                        self.muc.contactos
            else:
                ...
                if not jid in self.roster.contactos:
                    self.roster.contactos.append(jid)
                    print 'anadiendo (offline) ', jid, ' de ', \
                        self.roster.contactos
                else:
                    print 'no anadiendo (offline) ', jid, ' de ', \
                        self.roster.contactos
        else:
...
class ListaDeContactos:
    ...
    def __init__(self):
        ...
        try:
            ...
            print 'No se puede crear la tabla de contactos... asumo que esta creada. ', \
                sys.exc_info()[0]
    ...
    def getNick(self, transportname, longname):
        ...
        c = self.contact.select(sqlobject.AND(self.contact.q.transport
                                 == transportname,
                                self.contact.q.username
                                 == str(longname)))
        if c.count():
            nick = c[0].nick
        else:
    ...
    def setNick(
        ...
        if c.count():
            ...
            c = self.contact(transport=transportname,
                             username=str(longname), nick=str(nick))
        return 'Tu nuevo nick es: %s' % self.getNick(transportname,
                longname)
...
class bot:
    ...
    def broadcastMessage(
        ...
        for t in transportdestino:
            print '%s.broadcastMessage(transport=%s, nick=%s, fullname=%s, text=%s)'\
                 % (t.name, transport.name, nick, fullname, text)
    ...
    def message_from(
        ...
        print 'bot::message_from(transport=%s, text=%s, user=%s)'\
             % (transport.name, text, user)
        ...
        if comando == 'contactos' or comando == 'names':
            ...
            for t in self.transportlist:
                if t.contactos:
                    ...
                    for n in t.contactos:
                        mensaje += ' '\
                             + self.listacontactos.getNick(t.name, n)
            return mensaje
        elif comando == 'nick':
            ...
            if len(line) != 2:
                return 'Tu nick actual es '\
                     + self.listacontactos.getNick(transport, userid)\
                     + '. Escribe !nick [nuevo_nick] para cambiarlo.'
            else:
                nick = line[1]
                return self.listacontactos.setNick(transport, userid,
                        nick)
        else:
...
def loop():
    ...
    for C in CONFIGS:
        ...
        if 'JABBER_ID' in C and 'JABBER_PASS' in C:
            ...
            if 'JABBER_ROOM' in C:
                mucTransport = MucTransport(b, jabberBase,
                        C['JABBER_ROOM'])

        if 'PASSPORT_ID' in C and 'PASSPORT_PASS' in C:
            msnTransport = MsnTransport(b, C['PASSPORT_ID'],
                    C['PASSPORT_PASS'])
        ...
        if C['WELCOME_MSG']:
            ...
            b.broadcastMessage(jabberTransport, ['all', 'web'], '['
                                + C['WELCOME_MSG'] + ']', 'bot.py')

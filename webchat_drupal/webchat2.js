...
var GetChaturl = "/mis_modulos/webchat2/getChatData.php";
var SendChaturl = "/mis_modulos/webchat2/sendChatData.php";
...
function handlehHttpReceiveChat() {
  if (httpReceiveChat.readyState == 4) {
    ...
    if (results.length >= 4) {
	    for(i=0;i <= (results.length-4);i=i+4) { //goes through the result one message at a time
		    lastID = results[i];
...
// 	document.forms['chatoutput'].elements['outputList'].value += '\n'+ liName + ': ' + liText;
// }

//function insertNewContent(liName,liText) {
//	ptr = document.forms['chatoutput'].elements['outputList'];
//	ptr.value += '\n'+ liName + ': ' + liText;
//	ptr.scrollTop=ptr.scrollHeight;
//}

function insertNewContent(liDate, liName,liText) {
	var d = document.createElement("div");
	liText = liText.replace(/\n/g,"&#8629;<br/>")
	regexp = /(http:\/\/|https:\/\/|ftp:\/\/)([^\s]{1,15})([^\s]*)/ig
	liText = liText.replace(regexp,"<a href=\"$1$2$3\" title=\"Ir a $1$2$3\">$2...</a>");
	liDate = liDate.substring(liDate.lastIndexOf(" ") + 1);
	d.innerHTML = '<em style="color: blue">(' + liDate+ ')</em> <b style="color: blue">' + liName + ":</b> " + liText;
	var p = document.getElementById("outputlist");
	p.appendChild(d);
...
function checkName() {
	...
	if (currentName.value == '') {
		currentName.value = 'Invitado_'+ Math.floor(Math.random() * 10000);
	}
}
